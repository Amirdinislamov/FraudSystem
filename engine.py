...
from schemas import TransactionPayload, ClientProfileState, FraudDecision, DecisionStatus, MccStats
from rules import BaseFraudRule

class InMemoryProfileRepository:
    def __init__(self):
        self._db: Dict[str, ClientProfileState] = {}

    async def get(self, client_id: str) -> ClientProfileState:
        return self._db.get(client_id, ClientProfileState(client_id=client_id))

    async def save(self, profile: ClientProfileState):
        self._db[profile.client_id] = profile

class FraudScoringEngine:
    def __init__(self, repo: InMemoryProfileRepository, rules: List[BaseFraudRule]):
        self.repo = repo
        self.rules = rules
        ...
        try:
            ...
            print("⚠️ ВНИМАНИЕ: Файл fraud_ml_model.pkl не найден. Работаем по старинке.")
            self.model = None

    async def process(self, tx: TransactionPayload) -> FraudDecision:
        state = await self.repo.get(tx.client_id)
        receiver_state = await self.repo.get(tx.receiver_id) if tx.receiver_id else None
        ...
        if self.model:
            ...
            fraud_prob = self.model.predict_proba(X_input)[0][1]
        ...
        if decision == DecisionStatus.APPROVED:
            ...
            if receiver_state:
                ...
                await self.repo.save(receiver_state)
...
update_IS_FRaud_Error_System = True 
